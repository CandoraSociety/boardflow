...
import { Calendar, FileText, Users, Target, Clock, ChevronRight, AlertCircle } from "lucide-react";
...
export default function Dashboard() {
  ...
  const statCards = [
    { label: "Upcoming Meetings", value: upcomingMeetings.length, icon: Calendar, color: "bg-blue-50 text-blue-700", link: "/meetings" },
    { label: "Active Board Members", value: members.length, icon: Users, color: "bg-green-50 text-green-700", link: "/members" },
    { label: "Strategic Goals", value: goals.length, icon: Target, color: "bg-purple-50 text-purple-700", link: "/strategic-plan" },
    { label: "Documents Filed", value: docs.length, icon: FileText, color: "bg-amber-50 text-amber-700", link: "/documents" },
  ];
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(({ label, value, icon: Icon, color, link }) => (
          <Link key={label} to={link} className="bg-card rounded-xl border border-border p-5 hover:shadow-md transition-shadow group">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/meetings" className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1">View all <ChevronRight size={12} /></Link>
          </div>
          {nextMeeting ? (
            <div>
              ...
              <div className="flex gap-2 mt-4">
                <Link to={`/meetings/${nextMeeting.id}/agenda`} className="text-xs bg-primary text-primary-foreground px-3 py-1.5 rounded-lg hover:opacity-90 transition">Build Agenda</Link>
                <Link to={`/meetings/${nextMeeting.id}/minutes`} className="text-xs border border-border text-foreground px-3 py-1.5 rounded-lg hover:bg-muted transition">Take Minutes</Link>
              </div>
            </div>
          ) : (
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/strategic-plan" className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1">View all <ChevronRight size={12} /></Link>
          </div>
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/documents" className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1">View all <ChevronRight size={12} /></Link>
          </div>
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: "Schedule Meeting", to: "/meetings/new", icon: Calendar },
              { label: "Upload Document", to: "/documents", icon: FileText },
              { label: "Add Board Member", to: "/members", icon: Users },
              { label: "Board Assistant", to: "/board-assistant", icon: AlertCircle },
