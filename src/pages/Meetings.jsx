...
import { Plus, Calendar, Clock, MapPin, ChevronRight, Trash2 } from "lucide-react";
        ...
        <div className="space-y-8">
                    ...
                    <div key={meeting.id} className="bg-card border border-border rounded-xl p-5 hover:shadow-sm transition group">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Link to={`/meetings/${meeting.id}/agenda`} className="text-xs border border-border px-3 py-1.5 rounded-lg hover:bg-muted transition text-foreground">Agenda</Link>
                          <Link to={`/meetings/${meeting.id}/minutes`} className="text-xs border border-border px-3 py-1.5 rounded-lg hover:bg-muted transition text-foreground">Minutes</Link>
                          <button onClick={() => handleDelete(meeting.id)} className="text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition">
                            <Trash2 size={15} />
                          </button>
                        </div>
                      </div>
          ...
          {meetings.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm">Schedule your first board meeting to get started.</p>
            </div>
          )}
        </div>
      )}

      {showNew && <NewMeetingModal onClose={() => setShowNew(false)} onSaved={(m) => { setMeetings(prev => [m, ...prev]); setShowNew(false); }} />}
    </div>
  );
}