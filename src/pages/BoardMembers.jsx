...
export default function BoardMembers() {
  ...
  const load = async () => {
    const data = await base44.entities.BoardMember.list();
    setMembers(data);
    setLoading(false);
  };
      ...
      {showForm && (
        <form onSubmit={handleSave} className="bg-card border border-border rounded-xl p-5 mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm font-medium mb-1.5 block">Term Start</label>
              <input type="date" value={form.term_start} onChange={e => setForm({...form, term_start: e.target.value})}
                className="w-full border border-input rounded-lg px-3 py-2 text-sm bg-background focus:outline-none" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1.5 block">Term End</label>
              <input type="date" value={form.term_end} onChange={e => setForm({...form, term_end: e.target.value})}
                className="w-full border border-input rounded-lg px-3 py-2 text-sm bg-background focus:outline-none" />
            </div>
            <div className="sm:col-span-2">
              <label className="text-sm font-medium mb-1.5 block">Committee</label>
              <input value={form.committee} onChange={e => setForm({...form, committee: e.target.value})}
                className="w-full border border-input rounded-lg px-3 py-2 text-sm bg-background focus:outline-none" placeholder="e.g. Finance, Governance" />
            </div>
            <div className="sm:col-span-2">
        ...
        <div className="space-y-6">
                    ...
                    <div key={member.id} className="bg-card border border-border rounded-xl p-4 hover:shadow-sm transition group">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                          <button onClick={() => toggleStatus(member)} className="p-1.5 text-muted-foreground hover:text-foreground rounded" title={member.status === "active" ? "Deactivate" : "Activate"}>
                            {member.status === "active" ? <CheckCircle size={14} className="text-green-600" /> : <Circle size={14} />}
                          </button>
                          <button onClick={() => handleDelete(member.id)} className="p-1.5 text-muted-foreground hover:text-destructive rounded">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
          ...
          {members.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm">Add your first board member to get started.</p>
            </div>
          )}
        </div>
      )}
