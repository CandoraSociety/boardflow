...
import { Upload, FolderOpen, FileText, Search, Trash2, ExternalLink, Filter } from "lucide-react";
...
const DOC_TYPES = [
  ...
  { value: "ed_report", label: "ED Reports" },
  ...
  { value: "corporate_doc", label: "Corporate Documents" },
  { value: "other", label: "Other" },
];

const TYPE_ICONS = {
  minutes: "📋", agenda: "📅", financial_report: "💰", ed_report: "📊",
  committee_report: "👥", policy: "📜", bylaw: "⚖️", strategic_plan: "🎯",
  training: "📚", corporate_doc: "🏢", other: "📄",
};
...
export default function Documents() {
  ...
  const load = async () => {
    const data = await base44.entities.BoardDocument.list("-created_date");
    setDocs(data);
    setLoading(false);
  };
  ...
  const handleUpload = async (e) => {
    ...
    const doc = await base44.entities.BoardDocument.create({
      ...uploadForm,
      file_url,
      file_name: file.name,
    });
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input value={uploadForm.title} onChange={e => setUploadForm({...uploadForm, title: e.target.value})}
                className="w-full border border-input rounded-lg px-3 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring" placeholder="e.g. June 2026 Financial Report" />
            </div>
            <div>
      ...
      <div className="flex gap-2 flex-wrap mb-6">
        {DOC_TYPES.map(type => (
          <button key={type.value} onClick={() => setFilter(type.value)}
            className={`text-xs px-3 py-1.5 rounded-full border transition ${filter === type.value ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:text-foreground hover:bg-muted"}`}>
            {type.label}
          </button>
        ))}
      </div>
            ...
            <div key={doc.id} className="bg-card border border-border rounded-xl p-4 hover:shadow-sm transition group">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                  ...
                  <button onClick={() => handleDelete(doc.id)} className="p-1.5 text-muted-foreground hover:text-destructive rounded">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
